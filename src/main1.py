...
class MainWindow(QMainWindow, Ui_MainWindow):
    ...
    def open_face(self):
        work_d = os.getcwd()
        #work_d = work_d + "/face"
        work_d = "'{}'".format(work_d)
        path_d = "'{}'".format(path)
        #processdata1.__main__.run(["--input_path", path_d, "--output_path", work_d])
        #
        # 
        subprocess.run(["open_dbm/OpenFace_2.2.0_win_x64/FaceLandmarkVidMulti.exe","-f", path, "-vis-aus", "-vis-track"])
        #f=face(self)
        #os_print = str("python " + "open_dbm/process_data1.py " + "--input_path " +  "'{}'".format(path) + " --output_path " + "'{}'".format(work_d))
        #os_print = str("python " + "open_dbm/process_data1.py " + "--input_path " +  path + " --output_path " + work_d)
        #os_print = str("python " + "open_dbm/process_data1.py " + "--input_path " +  "C:/Users/chags/OneDrive/Desktop/vid.mp4" + " --output_path " + "C:/Users/chags/Downloads/mediaplayer/mediaplayer/processed")
        #print(os_print)
        #os.system(os_print)
        #facial = "'{}".format('facial')
        ### need to figure this out subprocess.run(["python", "open_dbm/process_data1.py","--input_path", path, "--output_path", work_d])
        #subprocess.Popen(["open_dbm/OpenFace_2.2.0_win_x64/FaceLandmarkVidMulti.exe", ])
        #(["python", "open_dbm/process_data1.py", "--input_path",  path, "--output_path", work_d])
        #subprocess.Popen(["python", " open_dbm/process_data1.py", " --input_path ", path, " --output_path ", work_d])
    def head(self):
        work_d = os.getcwd()
    ...
    def trn_dlg(self):
        ...
        self.file = open(self.load_prjct_file, 'a+')
        self.file.write("\n")
        self.file.write('Classifier trained, analyzed, and saved.' + " " + date_time)
        self.file.close()
    def click_data(self):
