# -*- coding: utf-8 -*-
"""
Created on Tue Apr 20 01:01:27 2021

@author: chags
"""

from dbm_lib.config import config_reader, config_raw_feature, config_derive_feature
from open_dbm.dbm_lib.controller import process_feature as pf
from open_dbm.dbm_lib.dbm_features.raw_features.video import open_face_process as of
from open_dbm.dbm_lib.dbm_features.derived_features import derive as der
...
OPENFACE_PATH_VIDEO = "OpenFace_2.2.0_win_x64/FaceLandmarkVid.exe"
OPENFACE_PATH = "OpenFace_2.2.0_win_x64/FeatureExtraction.exe"
DEEP_SPEECH = "C:/Users/chags/anaconda3/Scripts/deepspeech.exe"
DLIB_SHAPE_MODEL = 'pkg/shape_detector/shape_predictor_68_face_landmarks.dat'

output_path = "/face_output"
input_path = "C:/Users/chags/OneDrive/Desktop/vid.mp4"
def common_video(video_file, args, r_config):
    ...
    out_path = os.path.join(output_path, 'raw_variables')
...
def process_raw_video_file(args, s_config, r_config):
    ...
    try:
        if output_path != None:
            video_file = glob.glob(input_path)
...
def process_raw_audio_file(args, s_config, r_config):
    ...
    try:
        if output_path != None:
            audio_file = glob.glob(input_path)
...
def process_raw_video_dir(args, s_config, r_config):
    ...
    """
    if output_path != None:
        vid_loc = glob.glob(input_path + '/*.mp4') + glob.glob(input_path + '/*.mov') + glob.glob(input_path + '/*.MOV')
...
def process_raw_audio_dir(args, s_config, r_config):
    ...
    """
    if output_path != None:
        audio_loc = glob.glob(input_path + '/*.wav') + glob.glob(input_path + '/*.mp3') + glob.glob(input_path + '/*.MP3')
...
def process_derive(args, r_config, d_config, input_type):
    ...
    """
    if input_type == 'file':
        input_file = glob.glob(input_path)
    else:
        input_file = glob.glob(input_path + '/*')

    out_raw_path = os.path.join(output_path, 'raw_variables')
    out_derive_path = os.path.join(output_path, 'derived_variables')
